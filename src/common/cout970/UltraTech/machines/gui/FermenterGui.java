...
public class FermenterGui extends GuiContainer{
	...
	protected void drawGuiContainerBackgroundLayer(float f, int i, int j) {
		...
		if(!w){
			...
			Icon ic = water.getFluid().getStillIcon();
		...
		if(!ju){
			Minecraft.getMinecraft().renderEngine.bindTexture(TextureMap.locationBlocksTexture);
			Icon ic = juice.getFluid().getStillIcon();			int a = juice.amount*40/entity.getTankInfo(ForgeDirection.UP)[1].capacity;
		...
		this.drawCenteredString(fontRenderer, "Fermenter", xStart+95, yStart+6, UT_Utils.RGBtoInt(255, 255, 255));
	}
    ...
    protected void drawGuiContainerForegroundLayer(int x, int y) {
        ...
        if(UT_Utils.isIn(x, y, xStart+45, yStart+20, 20, 40)){
        	...
        	this.drawHoveringText(fluid1, x-xStart, y-yStart, fontRenderer);
        }

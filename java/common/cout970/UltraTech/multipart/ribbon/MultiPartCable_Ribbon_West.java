...
public class MultiPartCable_Ribbon_West extends MultiPartCable_Ribbon{


	@Override
	public String getType() {
		return base.getUnlocalizedName()+".west";
	}
	...
	public Cuboid6[] setupBoundingBoxes() {
		...
		boundingBoxes[0] = new Cuboid6(0, 0.5-w,  0.5-w, h,  0.5+w,  0.5+w);	//base 
		boundingBoxes[3] = new Cuboid6(0, 0,      0.5-w, h,  0.5-w,  0.5+w);	//north 2 == bottom
		boundingBoxes[2] = new Cuboid6(0, 0.5+w,  0.5-w, h,  1, 	   0.5+w);	//south 3 == top
		boundingBoxes[4] = new Cuboid6(0, 0.5-w,  0,     h,  0.5+w,  0.5-w);	//west 4 == west
		boundingBoxes[5] = new Cuboid6(0, 0.5-w,  0.5+w, h,  0.5+w,  1);		//east 5 == east
	...
	public void updateConnections(){
		if(tile() == null)return;
		if(!MultipartUtil.isSolid(getTile(), ForgeDirection.WEST) && !world().isRemote){
		...
		for(ICable c : NetworkManagerRegistry.getConnections(tile())){
			if(c != this.getPower().getCable()){
				ForgeDirection side = c.getInternalConection(CableType.Ribbon_West, getPower().getCable());
		...
		for(ForgeDirection d : ForgeDirection.VALID_DIRECTIONS){
			if(d == ForgeDirection.EAST)continue;

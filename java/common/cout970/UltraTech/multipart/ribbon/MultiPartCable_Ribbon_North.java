...
public class MultiPartCable_Ribbon_North extends MultiPartCable_Ribbon{

	@Override
	public String getType() {
		return base.getUnlocalizedName()+".north";
	}
	...
	public Cuboid6[] setupBoundingBoxes() {
		...
		boundingBoxes[0] = new Cuboid6(0.5-w, 0.5-w,  0, 0.5+w, 0.5+w, h);	//base 
		boundingBoxes[3] = new Cuboid6(0.5-w, 0,  0,     0.5+w, 0.5-w, h);	//north 2 == bottom
		boundingBoxes[2] = new Cuboid6(0.5-w, 0.5+w,  0, 0.5+w, 1, h);		//south 3 == top
		boundingBoxes[4] = new Cuboid6(0,     0.5-w,  0, 0.5-w, 0.5+w, h);	//west 4 == west
		boundingBoxes[5] = new Cuboid6(0.5+w, 0.5-w,  0, 1,     0.5+w, h);	//east 5 == east
	...
	public void updateConnections(){
		if(tile() == null)return;
		if(!MultipartUtil.isSolid(getTile(), ForgeDirection.NORTH) && !world().isRemote){
		...
		for(ICable c : NetworkManagerRegistry.getConnections(tile())){
			if(c != this.getPower().getCable()){
				ForgeDirection side = c.getInternalConection(CableType.Ribbon_North, getPower().getCable());
		...
		for(ForgeDirection d : ForgeDirection.VALID_DIRECTIONS){
			if(d == ForgeDirection.SOUTH)continue;

...
public class BlockWindMill extends BlockConductor{

	public BlockWindMill(Material par2Material) {
		super(par2Material);
	...
	public boolean onBlockActivated(World w, int x, int y, int z, EntityPlayer p, int par6, float par7, float par8, float par9)
	{
		...
		TileEntityWindMill e = (TileEntityWindMill) w.getTileEntity(x, y, z);
		if(e != null){
	...
	public void onBlockAdded(World par1World, int xCoord, int yCoord, int zCoord)
	{
		...
		for(int d=1;d<5;d++){
			TileEntityHitBox h =((TileEntityHitBox)par1World.getTileEntity(xCoord, yCoord+d, zCoord));
			if(h != null){
				h.x = xCoord;
				h.y = yCoord;
				h.z = zCoord;

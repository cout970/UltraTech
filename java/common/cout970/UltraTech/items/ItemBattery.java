...
public class ItemBattery extends ItemPower{
	...
	public boolean onItemUse(ItemStack stack, EntityPlayer player, World world, int x, int y, int z, int side, float hitX, float hitY, float hitZ){
		...
		if(t instanceof TileEntityBatteryTier1 || t instanceof TileEntityBatteryTier2 || t instanceof TileEntityBatteryTier3 || t instanceof TileEntityBatteryTier4){
	...
	public ItemStack onItemRightClick(ItemStack stack, World w, EntityPlayer p)
    {
		if(!p.isSneaking()){
			...
			for(ItemStack s : i){
				if(s != null){
					Item it = s.getItem();
					if(it instanceof IStorageItem && !(it instanceof ItemBattery)){
